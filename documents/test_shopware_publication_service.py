...
class DocumentShopwarePublicationServiceTests(SimpleTestCase):
    ...
    def test_publish_requires_both_selected_shopware_links(self):
        service = DocumentShopwarePublicationService.__new__(DocumentShopwarePublicationService)

        with self.assertRaisesMessage(ValueError, "Erlebniswelt auswählen"):
            service.publish(Document(title="AGB"))

        ...
        with microtech_connection() as erp:
            ...
            if sync_all:
                ...
                while not artikel_service.range_eof():
                    erp_nr = artikel_service.get_field("ArtNr")
                    if erp_nr:
                        erp_nrs.append(str(erp_nr).strip())
            ...
            for index, erp_nr in enumerate(erp_nrs, start=1):
                try:
                    ...
                    storage.stock = lager_service.get_stock(art_nr=product.erp_nr)
                    storage.location = lager_service.get_storage_location(art_nr=product.erp_nr)
                    storage.save()
                    ...
                    if image_names:
                        ...
                        for image_name in image_names:
                            image, _ = Image.objects.get_or_create(path=image_name)
                            images.append(image)
                    ...
                    success_count += 1
                    logger.info("Artikel {} synchronisiert.", product.erp_nr)
